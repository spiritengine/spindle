...
- `install-service` refuses to overwrite a service file spindle did not write,
  even with `--force`, instead of clobbering a hand-written unit. Service files
  generated by 1.1.0 and earlier carry no marker but are still recognized as
  spindle's own, so upgrading is not a dead end.
